...
import { alert as vbenAlert, prompt } from '@vben-core/popup-ui';

interface ToolbarGroupOptions {
  imageUploader?: TiptapImageUploader;
}
  ...
  if (imageUploader) {
    ...
    if (file) {
      if (!file.type.startsWith('image/')) {
        await vbenAlert({
          content: '请选择 JPG、PNG、WEBP、GIF 图片',
          title: '图片格式不支持',
        });
        return;
      }
...
async function handleLocalImageAction(
  editor: Editor,
  imageUploader: TiptapImageUploader,
) {
  const file = await selectLocalImageFile();
...
function createImageToolbarAction(
  imageUploader?: TiptapImageUploader,
): ToolbarAction {
  if (!imageUploader) {
    return {
  ...
  return {
    ...
    menu: {
      items: [
        {
          action: (editor) => handleLocalImageAction(editor, imageUploader),
          label: '从本地上传',
          shortLabel: 'UPL',
        },
        {
...
export function createToolbarGroups(
  ...
  return [
    ...
    [
      ...
      createImageToolbarAction(options.imageUploader),
    ],
    [
      {
        action: () => {},
