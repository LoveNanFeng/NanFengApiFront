...
const packageData = ref<PackageApi.InterfacePackage>();
...
const [Drawer, drawerApi] = useVbenDrawer({
  async onConfirm() {
    ...
    drawerApi.lock();
    openInterfacePackage(packageData.value.id, {
      expireTime: formatTime(formState.expireTime),
      specId: formState.specId,
  ...
  async onOpenChange(isOpen) {
    if (!isOpen) return;
    packageData.value = drawerApi.getData<PackageApi.InterfacePackage>();
    formState.userId = undefined;
    formState.specId = packageData.value?.specs?.[0]?.id;
...
const specOptions = computed(() =>
  (packageData.value?.specs ?? []).map((item) => ({
    label: item.specName,
    value: item.id,
  })),
);
  ...
  <Drawer :title="drawerTitle">
    <div class="open-package-form">
      ...
      <label>
        ...
        <DatePicker v-model:value="formState.startTime" class="w-full" show-time />
      </label>
      <label>
        <span>{{ $t('system.package.expireTime') }}</span>
        <DatePicker v-model:value="formState.expireTime" class="w-full" show-time />
      </label>
